...
気持ちをキャラクターに託した投稿や、創作練習のキャラクターの台詞が、誰かの物語と静かに共鳴する。
...
### 3. バックエンド：Rails Way（Omakase）の準拠

技術的負債を抑えるため、「素の Rails」に近い設計を選択。

- **工夫点**: 管理画面に **Avo**、メールに **Resend** を採用するなど、エコシステムを活用し、コア機能（ロールプレイ体験）の実装にリソースを集中させました。
...
- **インフラ強化**: RenderからAWSへの完全移行によるスケーラビリティ確保。
